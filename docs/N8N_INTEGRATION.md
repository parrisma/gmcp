...
This guide shows how to integrate gplot with N8N for automated chart generation.

## Prerequisites

- gplot MCP or Web server running
...
```
[Schedule] → [Database Query] → [Function (format data)] → [HTTP Request (gplot)] → [Send Email with Chart]
```

### Multi-Chart Dashboard

```
[Trigger] → [Split Data] → [HTTP Request (gplot)] × 3 → [Merge Images] → [Upload to Cloud]
```
...
### Connection Refused
- Ensure gplot server is running
- Check correct port (8000 for Web, 8001 for MCP)
...
- See `docs/GRAPH_PARAMS.md` for all parameters
