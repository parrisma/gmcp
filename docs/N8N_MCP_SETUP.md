# N8N MCP Client Setup for gplot

## Quick Start: Connect N8N to gplot MCP Server

Your gplot MCP server is running on **Streamable HTTP** which is fully compatible with N8N's MCP Client Tool!

### Prerequisites

✅ gplot MCP server running on `localhost:8001`  
...
- **URL**: `http://localhost:8001/mcp/` (or `http://gplot_dev:8001/mcp/` if in Docker network)
...
- ✅ Use correct URL: `http://localhost:8001/mcp/` (note trailing slash)
...
### Tool Not Found
**Issue**: MCP Client doesn't show gplot tools
...
- 📚 See `GRAPH_PARAMS.md` for all chart parameters
