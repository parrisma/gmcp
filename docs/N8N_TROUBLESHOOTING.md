...
## Common Error: "Error in sub-node 'MCP GPLOT Client'"
...
2. **URL**: `http://localhost:8000/render` (use Web API, not MCP)
3. **Method**: POST
...
python -m app.main_mcp --no-auth --host 0.0.0.0 --port 8001

# Web Server without authentication  
python -m app.main_web --no-auth --host 0.0.0.0 --port 8000
```
...
1. **Verify URL format:**
   - Try: `http://localhost:8001/mcp/`
   - Try: `http://localhost:8001/mcp`
   - Try: `http://host.docker.internal:8001/mcp/` (if N8N in Docker)
...
1. **Method**: POST
2. **URL**: `http://localhost:8000/render`
3. **Headers**:
...
2. **Port 8000** (Web API) instead of Port 8001 (MCP)
...
**If you get "Error in sub-node 'MCP GPLOT Client'":**

1. ✅ Switch to HTTP Request node (recommended)
2. ✅ Use Web API on port 8000
